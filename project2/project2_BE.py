...
for i, bc_node in enumerate(BC_nodes):
    ...
    M[:,bc_node] = BC_vals[i]
    K[bc_node, bc_node]=1
#creating B for backwards euler
B= (1 / dt) * M + K
...
plt.title(f"Backwards Euler FE solution at T ={tf} nt={int(nt)} N={N}")
plt.legend()

#save plots
if save_plots:
    filename = f"BE_Solution_T={tf:.1f}_nt={int(nt)}_N={N}.png"
